...
def get_all_methods(obj, parent=''):
    ...
    for i, attr_name in enumerate(dir(obj)):
        ...
        try:
            ...
            continue
        if callable(attr):
            full_method_name = f"{i}:{parent} -> {attr_name}" if parent else attr_name
            print(full_method_name)
            methods.append(full_method_name)
        if hasattr(attr, '__dict__'):
            methods.extend(get_all_methods(attr, parent=f"{i}:{parent} -- {attr_name}" if parent else attr_name))
    return methods


with TypeDB.core_client("0.0.0.0:1729") as client:
    get_all_methods(client)
